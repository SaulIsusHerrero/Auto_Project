...
public class TestsNike extends Base {
    ...
    static By confirmChoiceCookiesLocator = By.xpath("//button[@data-testid='dialog-accept-button']");
    ...
    public static void cookiesPageElements() throws InterruptedException {
        ...
        // cambiar los tests que tengan "acceptCookiesLocator" por "confirmChoiceCookiesLocator")
        try {
            WebElement element = wait.until(ExpectedConditions.visibilityOfElementLocated(confirmChoiceCookiesLocator));
        } catch (Exception e) {
            System.out.println("El elemento no es visible dentro del tiempo esperado");
        }
        boolean cookiesPopUp = isDisplayed(confirmChoiceCookiesLocator);
        ...
        botonAceptarCookiesTexto = driver.findElement(confirmChoiceCookiesLocator).getText().trim().toLowerCase(Locale.ROOT);
    ...
    public static void acceptCookies() throws InterruptedException {
        ...
        Thread.sleep(1000);
        Assert.assertFalse(isDisplayed(confirmChoiceCookiesLocator), "No se ha cerrado el pop up de cookies");
